...
% 
% Output:
%   :param: bv (type array) - K x 1 vector
...
function [bv,sebv,R2v,R2vadj,errv,v,F] = olsgmm(lhv, rhv, lags, weight)
    ...
    if weight == -1  
        ...
        for indx = 1:N
            ...
            % check whether first column is all zeros (constant measure)
            if rhv(:,1) == ones(T,1) 
